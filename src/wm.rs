...
fn output_geometry_for_window(state: &MitosGuiState, window: &Window) -> Rectangle<i32, Logical> {
    ...
    // Fallback to primary output if window isn't mapped to an output yet
    if let Some(primary) = state.outputs.first() {
        if let Some(geom) = state.space.output_geometry(primary) {
            return geom;
        }
    }
...
/// Area not covered by the top bar or the dock, relative to a specific output.
pub fn usable_area_for_output(state: &MitosGuiState, output_geom: Rectangle<i32, Logical>) -> Rectangle<i32, Logical> {
    let top = state.shell.top_bar.map(|p| p.size.1).unwrap_or(0);
    let bottom = state.shell.dock.map(|p| p.size.1 + 20).unwrap_or(0);

    let height = (output_geom.size.h - top - bottom).max(1);
...
/// Legacy helper for single-monitor assumptions (used by other modules if needed).
...
pub fn request_maximize(state: &mut MitosGuiState, window: &Window) {
    ...
    // Multi-monitor aware: get the geometry of the monitor the window is on
    let out_geom = output_geometry_for_window(state, window);
    let area = usable_area_for_output(state, out_geom);
...
pub fn request_fullscreen(state: &mut MitosGuiState, window: &Window) {
    ...
    // Multi-monitor aware: fullscreen to the specific monitor's size and location
    let out_geom = output_geometry_for_window(state, window);
    ...
    tl.with_pending_state(|s| {
        ...
        s.size = Some(out_geom.size);
    });
    tl.send_configure();

    state.space.map_element(window.clone(), out_geom.loc, true);
}
...
pub fn snap(state: &mut MitosGuiState, side: SnapSide) {
    ...
    let (x, w) = match side {
        SnapSide::Left => (out_geom.loc.x, half),
        SnapSide::Right => (out_geom.loc.x + area.size.w - half, half),
    };
